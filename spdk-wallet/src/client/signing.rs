...
#[cfg(test)]
mod test {
    use bitcoin::{Network, Witness, secp256k1::{SecretKey, rand}};

    use crate::client::{SpClient, SpendKey, signing::verify_message};

    fn create_random_client() -> SpClient {
        use rand::prelude::*;
        let mut rng = thread_rng();

        let mut scan_sk_bytes: [u8; 32] = [0; 32];
        let mut spend_sk_bytes: [u8; 32] = [0; 32];
        rng.fill_bytes(&mut scan_sk_bytes);
        rng.fill_bytes(&mut spend_sk_bytes);
